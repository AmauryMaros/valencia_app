...
nugent_color_scheme = {'low':'#5ca3e6','intermediate':'#daeef0','high':'#e35f5f'}


data_all = pd.read_csv("data/nugent_plot_data.csv")
...
nugent_list = ['low','intermediate','high']
data_all['bottom_count'] = pd.Series([0.0 for x in range(len(data_all.index))], index=data_all.index)

for nugent_cat in nugent_list:
    print(nugent_cat)

    stacked_axs.bar(data_all['loc'],data_all[nugent_cat],width=bar_width,bottom=data_all['bottom_count'],color=nugent_color_scheme[nugent_cat],clip_on=False,zorder=2)
    data_all['bottom_count'] = data_all['bottom_count'] + data_all[nugent_cat]
...
stacked_axs.set_ylabel('Proportion of samples in Nugent cat.',fontsize=20)
...
#making the legend
high_rect = mpatches.Rectangle((-0.5,.9),width=.15,height=.1,linewidth=1,edgecolor='none',facecolor='#e35f5f',clip_on=False)
medium_rect = mpatches.Rectangle((-0.5,.8),width=.15,height=.1,linewidth=1,edgecolor='none',facecolor='#daeef0',clip_on=False)
low_rect = mpatches.Rectangle((-0.5,.7),width=.15,height=.1,linewidth=1,edgecolor='none',facecolor='#5ca3e6',clip_on=False)

stacked_axs.add_patch(high_rect)
stacked_axs.add_patch(medium_rect)
stacked_axs.add_patch(low_rect)


stacked_axs.text(-.55,0.95,s=r'High',fontsize=14,ha='right',va='center')
stacked_axs.text(-.55,0.85,s=r'Intermediate',fontsize=14,ha='right',va='center')
stacked_axs.text(-.55,0.75,s=r'Low',fontsize=14,ha='right',va='center')

# stacked_fig.savefig('nugent_barplot.pdf')
